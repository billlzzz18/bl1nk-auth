  ...
  **Agents** - A local-first Electron desktop app for AI-powered code assistance. Users create chat sessions linked to local project folders, interact with Claude in Plan or Agent mode, and see real-time tool execution (bash, file edits, web search, etc.)
  ...
  **Queries:**
  ...
  rm -rf ~/Library/Application\ Support/{APP_NAME_DEV}/
  ...
  defaults delete {APP_ID_DEV}
  defaults delete {APP_ID_PROD}
  ...
  **Dev vs Production App:**
  - Dev mode uses `{PROTOCOL_DEV}://` protocol
  - Dev mode uses separate userData path (`~/Library/Application Support/{APP_NAME_DEV}/`)
  ...
  - Create with: `xcrun notarytool store-credentials "{NOTARIZE_PROFILE}" --apple-id YOUR_APPLE_ID --team-id YOUR_TEAM_ID`
  ...
  1. Wait for notarization (2-5 min): `xcrun notarytool history --keychain-profile "{NOTARIZE_PROFILE}"`
  ...
  1. App checks `{CDN_BASE_URL}/latest-mac.yml` on startup and when window regains focus (with 1 min cooldown)
